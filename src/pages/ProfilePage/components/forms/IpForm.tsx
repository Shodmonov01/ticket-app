...
export function IpForm() {
    ...
    const onSubmit = async (values: z.infer<typeof ipFormSchema>) => {
        try {
            await api.post('/auth/api/distributor/assign/ip/', values)
    ...
    return (
        <div>
            <HeaderPartner title='ИП' path='/profile/organization-role' />

            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4 p-4 mt-10 bg-transparent'>
                    {currentStep === 1 && (
                        <>
                            <FormField
